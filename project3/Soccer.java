...
public class Soccer {
    ...
    public static void main ( String [ ] args ) throws SQLException
    {
        ...
        String your_userid; // = ADD ID HERE;
        String your_password; // = ADD PASSWORD HERE;
...
//Q1
    ...
    public static void Q2() throws SQLException {
        ...
        try {
            String querySQL = "SELECT id, playing_country, opposing_country, date, round FROM Games WHERE timestamp(date,time) > current_timestamp INTERSECT SELECT id, playing_country, opposing_country, date, round FROM Games WHERE timestamp(date,time) < current_timestamp+3 DAYS";
    ...
    //Q3
    //main function body for Q3, gets ID + country for the following PrintPlayers function
    public static void Q3() throws SQLException {
        ...
        try {
            String querySQL = "SELECT id, playing_country, opposing_country, date, round FROM Games WHERE timestamp(date,time) > current_timestamp";
    ...
    public static void getRefs(int matchID) throws SQLException {
        ...
        System.out.println("List of all referees");


        int sqlCode = 0;      // Variable to hold SQLCODE
        String sqlState = "00000";  // Variable to hold SQLSTATE
        HashMap<String, String> listOfNames = new HashMap<String, String>();
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int y_o_e = rs.getInt(2);
                String rcountry = rs.getString(3);
                listOfNames.put(rname, rcountry);

                System.out.println(rname + "  " + rcountry + "  (" + y_o_e + " years of experience)");
            }
            ...
            System.out.println("Error: Unable to retrieve list of referees");
        }

        // get ref name from user
        String refname;
        Scanner ref_name_scan = new Scanner(System.in);
        System.out.println("Which referee do you want to select? Type P to go to the previous menu");
        refname = ref_name_scan.nextLine();
        if (refname.equals("P")) {
            Q3();
        } else {
            System.out.println("Selected " + refname);

            // check if this ref exists
            if (!listOfNames.containsKey(refname)) {
                System.out.println("Error: No referee found with name " + refname);
                int userchoice = showMenu();
                loop(userchoice, statement);
            } else {
                // ensure that this ref is not from one of the participating countries
                try {
                    String query = "SELECT playing_country, opposing_country FROM Games WHERE id = " + matchID;
                    ...
                    if (rs1.next()) {
                        country1 = rs1.getString(1);
                        country2 = rs1.getString(2);
                    }

                    if (country1.equals(listOfNames.get(refname))) {
                        System.out.print("Error: Referee " + refname + " is from " + country1 + ", which is playing in the match.\n\n");
                        int userchoice = showMenu();
                        loop(userchoice, statement);
                    } else if (country2.equals(listOfNames.get(refname))) {
                        System.out.print("Error: Referee " + refname + " is from " + country2 + ", which is playing in the match.\n\n");
                        int userchoice = showMenu();
                        loop(userchoice, statement);
                    } else {
                        // insert ref
                        addRef(refname, matchID, your_userid, your_password);
                    }

                } catch (SQLException e3) {
                    sqlCode = e3.getErrorCode(); // Get SQLCODE
                    sqlState = e3.getSQLState(); // Get SQLSTATE

                    // Your code to handle errors comes here;
                    // something more meaningful than a print would be good
                    System.out.println("Code: " + sqlCode + "  sqlState: " + sqlState);
                    System.out.println(e3);
                    System.out.println("Error: Couldn't check selected game");
                }
            }
            statement.close();
            con.close();
    ...
    public static void addRef(String refname, int gameID, String id, String password) throws SQLException {
        // try to add this ref to the refs_game
        String url = "jdbc:db2://winter2023-comp421.cs.mcgill.ca:50000/cs421";
        String your_userid = id;
        ...
        Connection con = DriverManager.getConnection(url, your_userid, your_password);
        Statement statement = con.createStatement();
        System.out.println("Which role should referee " + refname+ " have in this game?");
        Scanner keyboard1 = new Scanner(System.in);
        String refRole = keyboard1.nextLine();

        //now enter the new info to the performs table
        try {
            String update1 = "INSERT INTO Refs_game VALUES (\'"+refname+"\',"+gameID+",\'"+refRole+"\')";
            //System.out.print(update1);
            statement.executeUpdate(update1);
            System.out.println("Referee "+refname+" was successfully assigned to game "+gameID);
        } catch (SQLException e5) {
            sqlCode = e5.getErrorCode(); // Get SQLCODE
            sqlState = e5.getSQLState(); // Get SQLSTATE

            // Your code to handle errors comes here;
            // something more meaningful than a print would be good
            System.out.println("Code: " + sqlCode + "  sqlState: " + sqlState);
            System.out.println(e5);
            System.out.println("Error: Referee "+refname+" was already assigned to game "+gameID);
        }
        statement.close();
        con.close();
    }
 ...
 //menu options /returning to menus
   public static int showMenu() {
     ...
     System.out.println("3.Add referee to match");
